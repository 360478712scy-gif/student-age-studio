...
def validate(value,people,removed,old,api):
    ...
    for key in list(out):
        ...
        row=out[key]
        if key not in people and key not in old:save_review.warn(api.ApiError, '恋爱配置必须关联模组人物。')
        ...
        if row['enabled'] and not (row['male'] or row['female']):save_review.warn(api.ApiError, '可恋爱时未选择适配主角。')
        for name,lo,hi,integer in [('minGrade',7,12,True),('minRelation',4,5,True),('minFavor',0,100000,False),('favorWeight',0,1,False),('attrWeight',0,1,False)]:
            v=row.get(name)
            if type(v) not in (int,float) or not math.isfinite(v) or not lo<=v<=hi or integer and int(v)!=v:save_review.warn(api.ApiError, '恋爱条件数值无效：'+name)
