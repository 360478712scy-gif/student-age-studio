...
def owned(store, project):
    ...
    read_json, safe_path, ApiError = api.read_json, api.safe_path, api.ApiError
    state = read_json(safe_path(project.path, STATE), {})
    if not isinstance(state, dict) or any(not isinstance(v, list) or any(not isinstance(k, str) or not k.isdigit() for k in v) for v in state.values()):
        raise ApiError('原版资源编辑记录损坏，请恢复备份后继续。', 422)
    return state


def visible_rows(store, project, table, local):
    if not project.original_mode:
        return local
    native = store.catalog_rows(table)
    keys = set(native) | set(owned(store, project).get(table, []))
    return {**copy.deepcopy(native), **{k:v for k,v in local.items() if k in keys}}


def preserve_rows(store, project, table, rows):
