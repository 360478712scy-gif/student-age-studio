...
def resources(game, cache_dir):
    game = Path(game)
    folder = game / 'StudentAge_Data/StreamingAssets/aa/StandaloneWindows64'
    if not folder.is_dir():
        raise FileNotFoundError('找不到游戏资源目录：' + str(folder))
    out = Path(cache_dir); out.mkdir(parents=True, exist_ok=True)
    ...
    if 'bubble' not in found or 'sprite' not in found['bubble']:
        ...
        for n in names[:200]:
            print('  ', n)
        raise FileNotFoundError("未找到原版气泡素材。")
