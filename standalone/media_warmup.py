...
class MediaWarmup:
    ...
    def files(self, roots, excluded):
        found = {}
        # Resolve() on Windows = GetFinalPathNameByHandle per path; cache it
        # per scan and fast-path the excluded-equality check via normcase so
        # normal paths skip the syscall. Manifest keys stay resolved strings.
        _cache = {}
        def _resolved(p):
            try:
        ...
        for item in excluded:
            try:
                excluded_norm.add(os.path.normcase(os.path.abspath(item)))
                excluded_norm.add(os.path.normcase(str(_resolved(item))))
            except (OSError, ValueError):
                pass
        def _is_excluded(p):
            try:
                if os.path.normcase(os.path.abspath(p)) not in excluded_norm:
                    return False
            except (OSError, ValueError):
                return False
            return _resolved(p) in excluded
        for root in roots:
