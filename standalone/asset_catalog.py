...
class AssetCatalog:
    ...
    def _hash_worker(self):
        ...
        while True:
            with self._hash_guard:
                if not self._hash_pending:
                    ...
                    if len(self.image_hashes) > 20000:
                        # LRU-style trim (was full clear): drop oldest quarter
            ...
            # Yield CPU to the editing UI; background dedup may finish later
            # on Windows but foreground priority is unchanged. Pixels unchanged.
            time.sleep(.05 if os.name == 'nt' else .01)
