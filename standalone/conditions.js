...
function label(row){const value=row?.name||row?.title||row?.content||row?.desc;return (Array.isArray(value)?value[0]:value)||'未命名内容';}
...
function summary(rows,templates,refs){return (rows||[]).map(row=>{const t=(templates||[]).find(t=>match(row,t));if(!t)return '自定义条件：'+row.join(',');return t.label+'：'+(t.parameters||[]).map(p=>p.range?.table?label(refs?.[p.range.table]?.[row[p.index]])||row[p.index]:p.choices?.find(c=>Number(c.id)===Number(row[p.index]))?.label??row[p.index]).join(' / ');}).join('，且 ');}
