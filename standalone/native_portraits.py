...
class Reader:
 ...
 def _members(self):
  # Built once per process: previously every environment() re-read all l2d
  # bundle headers (O(B^2) opens, brutal on NTFS+Defender). Shared read-only
  # map, same content, far fewer syscalls. Pixels unchanged.
 ...
 def environment(self,path):
  ...
  while len(self.environments)>self._MAX_ENVS:
   self.environments.pop(next(iter(self.environments)));gc.collect()
  return self.environments[path]
