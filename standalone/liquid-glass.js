...
(()=>{'use strict';
function start(){if(window.STUDIO_THEME==='classic'||window.STUDIO_GLASS)return;
...
const targets='button,#studio-toolbar,.wk-feature-card,.wk-rows,.guide-shell,.guide-folder,.guide-preference,.guide-toolbar-map article,.guide-contacts>div,.cache-upgrade,dialog[open],#studio-corner-tools button,.toast:not([hidden]),.character-list-panel,.character-content,.social-list,.social-settings-card,.space-list,.space-form,.messages-sidebar,.messages-settings,.goal-list,.goal-settings,.ap-controls,.ap-preview,dialog[open] .location-section,dialog[open] .workshop-settings-tabs,dialog[open] .condition-browser-catalog,dialog[open] .condition-browser-selected,dialog[open] .event-parameter-panel,dialog[open] .ap-folder,dialog[open] .ap-card,dialog[open] .condition-library-entry,dialog[open] .character-choice-grid>button,dialog[open] .minigame-detail,dialog[open] .minigame-list,dialog[open] .wk-command,dialog[open] .wk-array,dialog[open] .warehouse-command-grid>section';
const native='.story-scene,#story-history,#scene-modal,.goal-native-canvas,.message-native-phone,.social-feed-card,#space-preview,.model-art,.goal-preview-zoom,.messages-preview-dialog';
const ro=new ResizeObserver(()=>request());
function discover(){scanTimer=0;if(lost||window.STUDIO_THEME==='classic')return;for(const [el,p]of panes){if(!el.isConnected||!el.matches(targets)){ro.unobserve(el);p.canvas.remove();el.classList.remove('liquid-pane','liquid-relative','liquid-ready');panes.delete(el);}else if(p.canvas.parentElement!==el){el.append(p.canvas);p.key='';}}
for(const el of document.querySelectorAll(targets)){if(panes.has(el)||el.closest(native)||el.id==='studio-cache-details'||el.id==='studio-cache-progress')continue;const canvas=document.createElement('canvas');canvas.width=1;canvas.height=1;canvas.className='liquid-optics';canvas.setAttribute('aria-hidden','true');const position=getComputedStyle(el).position;if(position==='static')el.classList.add('liquid-relative');el.classList.add('liquid-pane');el.append(canvas);panes.set(el,{canvas,ctx:null,key:'',light:[-100,-100],hover:0});ro.observe(el);}
stats.panes=panes.size;request();}
function request(){if(window.STUDIO_THEME!=='classic'&&!raf&&!document.hidden&&!opaque.matches&&!lost)raf=requestAnimationFrame(draw);}
...
function scheduleScan(records){if(window.STUDIO_THEME==='classic')return;
...
// Mutation callbacks run before the next paint: attach and compose the glass right here, so a list that
// was rebuilt (select, expand, edit) never shows its plain fallback for a frame.
if(scanTimer){clearTimeout(scanTimer);scanTimer=0;}discover();if(raf){cancelAnimationFrame(raf);raf=0;}if(window.STUDIO_THEME!=='classic'&&!document.hidden&&!opaque.matches&&!lost)draw();}
new MutationObserver(scheduleScan).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['open','hidden']});
document.addEventListener('pointermove',e=>{if(window.STUDIO_THEME==='classic'||reduce.matches||opaque.matches||e.buttons)return;const el=e.target.closest?.('.liquid-pane'),p=panes.get(el);if(!p)return;const r=el.getBoundingClientRect();p.light=[e.clientX-r.left,e.clientY-r.top];p.hover=1;pointerRequest();},{passive:true});
document.addEventListener('pointerout',e=>{const el=e.target.closest?.('.liquid-pane'),p=panes.get(el);if(p&&!el.contains(e.relatedTarget)){p.light=[-100,-100];p.hover=0;pointerRequest();}},{passive:true});
...
document.addEventListener('scroll',e=>{scrolling=true;const p=panes.get(e.target);if(p)placeCanvas(e.target,p.canvas);request();clearTimeout(scrollTimer);scrollTimer=setTimeout(()=>{scrolling=false;request();},100);},{capture:true,passive:true});window.addEventListener('resize',request,{passive:true});document.addEventListener('visibilitychange',()=>{if(document.hidden&&raf){cancelAnimationFrame(raf);raf=0;}else request();});opaque.addEventListener('change',()=>{document.documentElement.classList.toggle('liquid-gpu',!opaque.matches&&window.STUDIO_THEME!=='classic');request();});
...
window.addEventListener('pagehide',()=>{if(raf)cancelAnimationFrame(raf);clearTimeout(scanTimer);clearTimeout(scrollTimer);clearTimeout(pointerTimer);ro.disconnect();gl.deleteTexture(texture);gl.deleteBuffer(buffer);gl.deleteProgram(program);},{once:true});
window.addEventListener('studio-theme-change',()=>{const enabled=window.STUDIO_THEME!=='classic';document.documentElement.classList.toggle('liquid-gpu',enabled&&!opaque.matches&&!lost);if(!enabled){if(raf)cancelAnimationFrame(raf);raf=0;for(const [el,p]of panes){p.canvas.width=1;p.canvas.height=1;p.key='';el.classList.remove('liquid-ready');}gpu.width=1;gpu.height=1;source.width=1;source.height=1;gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,1,1,0,gl.RGBA,gl.UNSIGNED_BYTE,null);width=0;height=0;stats.renderer='classic (paused)';}else{stats.renderer=lost?'context-lost fallback':'WebGL';width=0;height=0;allDirty=true;discover();}});
