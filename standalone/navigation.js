...
const mergeButton=document.createElement('button');mergeButton.id='studio-merge-corner';mergeButton.className='json-corner';mergeButton.type='button';mergeButton.title='合并模组';mergeButton.setAttribute('aria-label','合并模组');mergeButton.innerHTML=svg('<path d="M6 3v6a4 4 0 0 0 4 4h4a4 4 0 0 1 4 4v4M6 3a2 2 0 1 0 0 .01M18 21a2 2 0 1 0 0 .01M18 3v6a4 4 0 0 1-4 4"/>');
mergeButton.onclick=()=>{if(typeof window.STUDIO_MERGE_MODS!=='function'){window.STUDIO_REPORT_ERROR?.(Error('合并功能尚未就绪，请重新打开工作台。'));return;}window.STUDIO_MERGE_MODS().catch(error=>window.STUDIO_REPORT_ERROR?.(error));};
cornerItems.append(mergeButton,helpButton,settingsButton,sourceButton,pluginButton);
...
const projectLabels=[...bar.querySelectorAll('.studio-project')];bar.insertBefore(contextGroup,projectLabels[0]);contextGroup.append(...projectLabels,bar.querySelector('[data-nav=publish]'),bar.querySelector('[data-nav=refresh]'));
...
async function action(name){
 ...
 return run(async()=>{
  ...
  if(name==='publish'){if(!await prepareLeave())return;const project=(window.STUDIO_PROJECTS?.()||[]).find(p=>p.id===window.STUDIO_CURRENT_PROJECT?.());return window.STUDIO_PUBLISH.open({project,prepare:async()=>{if(window.STUDIO_HAS_UNSAVED_CHANGES?.())throw Error('请先保存当前修改。');}});}
