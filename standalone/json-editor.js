...
// Disk-backed story tables (large TalkCfg) are edited one segment at a time through the story draft.
function segmentFor(name){const wk=window.STUDIO_WORKSHOP_NAV;if(wk?.isOpen?.())return null;return window.STUDIO_STORY_JSON?.segment?.(name)||null;}
...
async function open(preferred){
 ...
 let context=id+'|'+hint()+'|'+(window.STUDIO_WORKSHOP_NAV?.isOpen?.()?'':window.STUDIO_STORY_JSON?.segmentKey?.()||''),loadSequence=0;
 ...
 function stash(){if(!state?.path||state.segmented)return;state.current=text.value;state.scrollTop=text.scrollTop;state.scrollLeft=text.scrollLeft;drafts.set(state.id+'|'+state.path,state);}
 ...
 function paintSegment(segment){const bar=q('[data-json-segment]');bar.hidden=!segment;if(!segment)return;q('[data-json-segment-label]').textContent=`分段读取 · ${segment.label}（共 ${segment.total} 条，每段 ${segment.ids.length<segment.total?segment.ids.length+' 条':'全部'}）`;q('[data-json-segment-page]').textContent=(segment.page+1)+' / '+segment.pages;q('[data-json-segment-prev]').disabled=segment.page<=0||state.busy;q('[data-json-segment-next]').disabled=segment.page>=segment.pages-1||state.busy;}
 async function loadSegment(path,newId,seq){const name=table(path),story=window.STUDIO_STORY_JSON;await story.prepare(name);if(!dialog||seq!==loadSequence)return;const segment=segmentFor(name),rows=story.read(name,{});if(!segment||rows==null)throw Error('对话正文尚未读取，请稍后重试。');const value=JSON.stringify(rows,null,2)+'\n';Object.assign(state,{path,loaded:value,current:value,bom:false,revision:null,segmented:true,segmentKey:segment.key,disk:{}});text.value=value;state.visual=JSON.stringify(rows);applyAnalysis({valid:true,errors:[]});paintSegment(segment);}
 async function load(path,newId=projectId()){stash();clearTimeout(timer);const seq=++loadSequence;state={id:newId,path:'',loaded:'',current:'',bom:false,revision:null,history:[],future:[],lastPush:0,errors:[],valid:true,busy:true,analysis:0};text.value='';refreshButtons();setStatus('正在读取…');paintSegment(null);try{const saved=drafts.get(newId+'|'+path);if(segmentFor(table(path))){await loadSegment(path,newId,seq);if(!dialog||seq!==loadSequence)return;}else if(saved){state=saved;state.busy=false;state.pending=false;text.value=state.current;}else{const file=list.files.find(f=>f.path===path);const data=file?.virtual?{text:'{}',revision:list.revision,analysis:{valid:true,errors:[]}}:await api('/api/json-source',{projectId:newId,path});if(!dialog||seq!==loadSequence)return;Object.assign(state,{path,loaded:data.text,current:data.text,bom:data.bom,revision:data.revision,virtual:!!file?.virtual});text.value=data.text;applyAnalysis(data.analysis);const parsed=await api('/api/json-analyze',{text:data.text,parse:true});if(!dialog||seq!==loadSequence)return;state.disk=parsed.valid?parsed.value:{};}
 ...
 async function save(force=false){if(state.busy||readOnly())return;await check();if(!force&&!state.valid&&!await ask('JSON 仍有语法错误，游戏可能无法读取这个文件。仍然保存？','仍然保存'))return;const current=state;state.busy=true;refreshButtons();setStatus('正在保存…');try{const value=text.value;
  if(state.segmented){const saveStory=window.STUDIO_SAVE_EVENT_CHANGES;if(typeof saveStory!=='function')throw Error('剧情编辑尚未就绪，请重新打开。');const result=await saveStory();if(!dialog||current!==state)return;if(result===false){setStatus('保存已取消，修改仍保留在当前段落。',true);return;}state.loaded=value;setStatus('已保存到模组。');return;}
 ...
 dialog.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.hasAttribute('data-json-close'))close();else if(b.hasAttribute('data-json-fix'))fix();else if(b.hasAttribute('data-json-check')){setStatus('正在检查…');check();}else if(b.hasAttribute('data-json-undo'))undo();else if(b.hasAttribute('data-json-redo'))redo();else if(b.hasAttribute('data-json-save'))save();else if(b.dataset.jsonGoto!==undefined)goto(Number(b.dataset.jsonGoto));else if(b.hasAttribute('data-json-segment-prev')||b.hasAttribute('data-json-segment-next'))turnSegment(b.hasAttribute('data-json-segment-next')?1:-1);});
 async function turnSegment(step){if(state.busy||!state.segmented)return;await check();if(!state.valid){setStatus('请先修正本段的语法错误，再切换段落。',true);return;}const segment=segmentFor(table(state.path));if(!segment)return;window.STUDIO_STORY_JSON.setSegmentPage(segment.page+step);context=projectId()+'|'+hint()+'|'+window.STUDIO_STORY_JSON.segmentKey();await load(state.path);text.scrollTop=0;}
