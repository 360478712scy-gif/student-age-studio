...
function controls(){if(!q('[data-guide-next]'))return;q('[data-guide-next]').disabled=working||step===0&&!ready;q('[data-guide-back]').disabled=working;q('[data-guide-skip]').disabled=working;q('[data-guide-skip]').textContent=step===0?'跳过等待':'暂时跳过';}
function shell(){
 root.dataset.first=String(first);root.classList.remove('brand-only');
 root.innerHTML=`<div class="guide-shell"><aside class="guide-identity"><img class="guide-logo" src="/icon.png" alt="拾光工坊图标"><h1>拾光工坊</h1><p>学生时代模组编辑器</p><span class="guide-version">${esc(window.STUDIO_VERSION||'')}</span><ol>${labels.map((v,i)=>`<li data-guide-step="${i}" ${i===step?'aria-current="step"':''}><span>${String(i+1).padStart(2,'0')}</span>${v}</li>`).join('')}</ol><small>拾起灵感，写下你的学生时代。</small></aside><section class="guide-content"><div class="guide-page" id="guide-page" tabindex="-1"></div><p class="guide-notice" data-onboarding-notice role="status" aria-live="polite"></p><footer class="guide-footer"><button data-guide-back>上一步</button><span></span><button data-guide-skip>跳过等待</button><button class="primary" data-guide-next>继续 <span aria-hidden="true">→</span></button></footer></section></div>`;
...
function render(){
 ...
 const page=q('#guide-page');page.classList.remove('guide-enter');
   q('[data-guide-back]').hidden=step===0;q('[data-guide-skip]').hidden=step!==0;q('[data-guide-next]').innerHTML=step===4?'进入拾光工坊 <span aria-hidden="true">→</span>':'继续 <span aria-hidden="true">→</span>';
 ...
 if(step===4){page.innerHTML='<span class="guide-eyebrow">'+esc(window.STUDIO_VERSION||'')+' · 公测邀请</span><h2>欢迎，一起拾起这段时光。</h2><p class="guide-description">感谢你参加拾光工坊 Beta 公测。无论是遇到 Bug、提出建议，还是交流模组创作，都欢迎告诉我们。</p><div class="guide-welcome-mark"><img src="/icon.png" alt="拾光工坊"></div><div class="guide-contacts"><div><span>反馈与编辑器讨论 QQ 群</span><strong>1121725122</strong><button data-guide-copy="1121725122">复制群号</button></div><div><span>也可以通过邮箱反馈</span><strong>360478712scy@gmail.com</strong><button data-guide-copy="360478712scy@gmail.com">复制邮箱</button></div></div><p class="guide-caption">反馈问题时，附上软件版本、操作步骤和错误日志，会更容易找到原因。</p>';page.querySelectorAll('[data-guide-copy]').forEach(n=>n.onclick=async()=>{try{await navigator.clipboard.writeText(n.dataset.guideCopy);notice('已复制。');}catch{notice('请复制：'+n.dataset.guideCopy);}});}
