...
def parser():
    ...
    c = cmd('edit-line', '修改一句对话的文字、说话人或演出（登场、表情、退场、背景、音效）'); c.add_argument('mod'); c.add_argument('talk_id'); c.add_argument('--text'); c.add_argument('--speaker'); c.add_argument('--display-name')
    ...
    c.add_argument('--actions', help='原版动作数组 JSON（整体替换）'); writes(c)
    ...
    c = cmd('assets', '查素材编号：backgrounds / music / sounds / expressions'); c.add_argument('mod'); c.add_argument('kind', choices=['backgrounds', 'music', 'sounds', 'expressions'])
...
def run(args):
    ...
    if c == 'edit-line': return studio.edit_line(args.mod, args.talk_id, args.text, args.speaker, args.display_name, **w(), background=args.background, enter=args.enter,
                                                 expression=args.expression, exit=args.exit, sound=args.sound, actions=_json_arg(args.actions, 'actions'))
