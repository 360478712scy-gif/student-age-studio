...
def parser():
    ...
    c = cmd('create-event', '新建事件及其对话。--lines 为对话数组 JSON：[{"speaker":"人物名|编号|旁白|主角","text":"…","enter":"左|中|右","options":[{"text":"选项","lines":[…]}]}]')
    ...
    c = cmd('edit-line', '修改一句对话的文字、说话人、演出、跳转、条件分支或效果'); c.add_argument('mod'); c.add_argument('talk_id'); c.add_argument('--text'); c.add_argument('--speaker'); c.add_argument('--display-name')
    c.add_argument('--enter', help='登场位置：左/中/右（"无" 取消）'); c.add_argument('--expression', help='表情名称或编号（空字符串取消）')
    c.add_argument('--exit', nargs='+', help='在这句退场的人物'); c.add_argument('--background', help='背景编号或名称'); c.add_argument('--sound', help='音效编号或名称（"无" 取消）')
    c.add_argument('--actions', help='原版动作数组 JSON（整体替换）'); c.add_argument('--cg', help='显示 CG（编号或名称），end 结束')
    c.add_argument('--next', help='之后跳到的对话编号，0 为结束'); c.add_argument('--check', help='判定条件 JSON'); c.add_argument('--next-if-failed', help='判定失败跳到的对话编号')
    c.add_argument('--effect', help='本句效果 JSON'); writes(c)
    c = cmd('set-options', '设置一句对话后的玩家选项（整体替换），--options 为 JSON：[{"text":"…","goto":对话编号 | "lines":[…]}]'); c.add_argument('mod'); c.add_argument('talk_id')
    c.add_argument('--options', required=True); c.add_argument('--rejoin-to'); writes(c)
    c = cmd('set-music', '给一段对话设置背景音乐（不写 --music 则清除）'); c.add_argument('mod'); c.add_argument('line_ids', nargs='+'); c.add_argument('--music')
    c.add_argument('--no-loop', action='store_true'); c.add_argument('--volume', type=float, default=1); writes(c)
    c = cmd('assets', '查编号：backgrounds / music / sounds / expressions / cgs / maps / event_types / dialogue_uses'); c.add_argument('mod')
    c.add_argument('kind', choices=['backgrounds', 'music', 'sounds', 'expressions', 'cgs', 'maps', 'event_types', 'dialogue_uses'])
    c.add_argument('-q', '--query'); c.add_argument('--person', help='查表情时的人物'); c.add_argument('--limit', type=int, default=40)
    ...
    c = cmd('update-rows', '新增、修改或删除配置表记录，--rows 为 {编号: {字段: 值}}'); c.add_argument('mod'); c.add_argument('name'); c.add_argument('--rows')
    c.add_argument('--delete', nargs='+', help='要删除的本模组记录编号'); writes(c)
    c = cmd('describe-table', '配置表字段说明；不写表名则列出全部表'); c.add_argument('mod'); c.add_argument('name', nargs='?')
    c = cmd('create-mod', '新建空模组，或 --copy-from 复制已有模组'); c.add_argument('name'); c.add_argument('--copy-from')
    c = cmd('import-asset', '导入本机图片或音频'); c.add_argument('mod'); c.add_argument('kind', choices=['background', 'cg', 'portrait', 'music', 'sound']); c.add_argument('file')
    c.add_argument('--name'); c.add_argument('--person'); c.add_argument('--face', type=int, default=0); c.add_argument('--cloth', type=int, default=0); c.add_argument('--grade', type=int, default=1)
    c = cmd('check', '检查 JSON 语法与剧情连接'); c.add_argument('mod')
    c = cmd('json', '列出、读取或整体替换模组的 JSON 文件（最后手段）'); c.add_argument('mod'); c.add_argument('path', nargs='?'); c.add_argument('--text', help='新内容：JSON 文本、@文件或 -')
    c.add_argument('--confirm-overwrite', action='store_true')
    c = cmd('external-dialogues', '列出事件外对话夹与用途'); c.add_argument('mod')
    c = cmd('create-external-dialogue', '新建事件外对话夹，可绑定用途'); c.add_argument('mod'); c.add_argument('--name', required=True); c.add_argument('--lines', required=True)
    c.add_argument('--uses', help='用途 JSON 数组，如 [{"kind":"gift","npc":"肖清雅","item":1000001}]'); writes(c)
...
def run(args):
    ...
    if c == 'edit-line': return studio.edit_line(args.mod, args.talk_id, args.text, args.speaker, args.display_name, **w(), background=args.background, enter=args.enter,
                                                 expression=args.expression, exit=args.exit, sound=args.sound, actions=_json_arg(args.actions, 'actions'), cg=args.cg,
                                                 goto=args.next, check=_json_arg(args.check, 'check'), next_if_failed=args.next_if_failed, effect=_json_arg(args.effect, 'effect'))
    if c == 'set-options': return studio.set_options(args.mod, args.talk_id, _json_arg(args.options, 'options'), args.rejoin_to, **w())
    if c == 'describe-table': return studio.describe_table(args.mod, args.name)
    if c == 'create-mod': return studio.create_mod(args.name, args.copy_from)
    if c == 'import-asset': return studio.import_asset(args.mod, args.kind, args.file, args.name, args.person, args.face, args.cloth, args.grade)
    if c == 'check': return studio.check_mod(args.mod)
    if c == 'json':
        text = None if args.text is None else sys.stdin.read() if args.text == '-' else Path(args.text[1:]).expanduser().read_text(encoding='utf-8') if args.text.startswith('@') else args.text
        return studio.json_file(args.mod, args.path, text, args.confirm_overwrite)
    if c == 'external-dialogues': return studio.external_dialogues(args.mod)
    if c == 'create-external-dialogue': return studio.create_external_dialogue(args.mod, args.name, _json_arg(args.lines, 'lines'), _json_arg(args.uses, 'uses'), **w())
    if c == 'set-music': return studio.set_music(args.mod, args.line_ids, {'id': args.music, 'loop': not args.no_loop, 'volume': args.volume} if args.music else None, **w())
    if c == 'assets': return studio.assets(args.mod, args.kind, args.query, args.person, args.limit)
    ...
    if c == 'update-rows': return studio.update_rows(args.mod, args.name, _json_arg(args.rows, 'rows'), **w(), delete=args.delete)
