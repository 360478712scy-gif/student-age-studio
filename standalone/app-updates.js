...
window.addEventListener('load',()=>{
 api('/healthy',{}).catch(()=>{});
 // A check never downloads or restarts. Only notify when a verified newer release exists.
 setTimeout(async()=>{try{const status=await api();if(!status.managed)return;const s=await api('/check',{});if(s.status==='available')window.STUDIO_NOTIFY?.('发现新版本 '+s.version+'，可在工坊设置 → 版本与反馈中更新。');}catch{}},12000);
},{once:true});
})();
