...
window.addEventListener('load',()=>{
 ...
 setTimeout(async()=>{try{const status=await api();if(!status.managed)return;const s=await api('/check',{});if(s.status==='available'){await api('/download',{});window.STUDIO_NOTIFY?.('正在后台下载 '+s.version+'，完成后可在工坊设置 → 版本与反馈中保存并重启更新。');}}catch{}},12000);
},{once:true});
})();
