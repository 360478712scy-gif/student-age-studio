...
function socialMount(node,state,parameters,options){
 ...
 node.onclick=async e=>{if(kind==='relation'&&e.target.closest('[data-social-action-clear]')){r.actionId=0;options.repaint();return;}if(!e.target.closest('[data-social-action]'))return;try{const rows=Object.values(parameters.actions||{}).filter(a=>kind!=='date'||Number(a.type)===3),chosen=await actionPicker(options.projectId,rows,r.actionId?[r.actionId]:[],{single:true});if(chosen){r.actionId=chosen[0]||0;options.repaint();}}catch(error){options.error(error);}};
 return true;
}
function validateSocial(doc,event,state,refs){const r=state.social;if(!r)return;if(!Number.isInteger(event.maxcount)||event.maxcount<1||event.maxcount>2147483647)throw Error('重复次数应为正整数。');if(!Number.isFinite(Number(r.favor))||Number(r.favor)<0||!Number.isFinite(Number(r.intimacy)))throw Error('请填写有效的好感度和亲密度数值。');if(r.kind==='talk'&&!r.text.trim())throw Error('请填写正在和谁一起做什么。');if((r.kind==='date'||r.kind==='relation'&&r.actionId)&&!({...refs.ActionCfg,...doc.actions})[r.actionId])throw Error('请选择行动。');if(r.kind==='date'){const a=({...refs.ActionCfg,...doc.actions})[r.actionId];if(Number(a.type)!==3)throw Error('请选择约会行动。');}
