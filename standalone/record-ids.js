...
async function open(table,oldId){if(working||draft)return;working=true;try{const data=await refresh();if(!data)throw Error('请先打开模组。');const row={projectId:project(),revision:data.revision,table,oldId:Number(oldId)},r=rule(table,oldId);draft=row;
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-record-table]');if(!b||b.disabled)return;e.preventDefault();e.stopImmediatePropagation();open(b.dataset.recordTable,b.dataset.recordId);},true);
...
window.STUDIO_IDS={editDraft,configure,refresh,ensure,allocate,rule,open,apply,history,canUndo:()=>canHistory('undo'),canRedo:()=>canHistory('redo'),get busy(){return working;}};
})();
