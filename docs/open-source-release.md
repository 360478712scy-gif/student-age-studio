...
1. 更新 `standalone/error_logs.py` 的 `APP_VERSION`，例如 `1.3.0-beta.2`。
2. 测试后将版本改动提交并推送到 main，工作流会为该版本生成标签并发布；也可手动推送匹配的 `v1.3.0-beta.2` 标签，或在 Actions 页面运行 `Publish code update`。
